...
_interupt = False  # флаг для прерывания потока
_last_state_key = False

def sleep_break(t=0, t2=0.1):
  """
...
# if keyboard.is_pressed('space') or interupt: break
...
def is_break():
  global _last_state_key, _interupt
  if _interupt:
...
def kb_hook_default(key):
  ...
  if key.event_type == "down":
    if key.name == "esc":
      ...
      _interupt = True # для прерывания потока
  ...
  while True:
    try:
      ...
      if state != b:
        state = b
        if b == True:
          log.info("%s, %s", b, _interupt)
        time.sleep(0.05)
    except BaseException as err: # Exception('Exit by hook Esc')
      log.info(err)
      # print(str(type(err)).removeprefix('<class ').removesuffix('>'), err)
      _interupt = False # для прерывания потока
  unhook()
